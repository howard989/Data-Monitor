...
function BlockProduction() {
  ...
  const renderTable = (title, interval, loadFunc) => {
    const isHourly = interval === 'hourly';
    const tableData = isHourly ? hourlyTable : dailyTable;
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 mb-6">
              ...
              <tbody>
                {tableData.rows.map((r, i) => (
                  <tr key={i} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                    <td className="py-2 px-2 text-gray-700">{r.block_date}</td>
        ...
        <div className="flex justify-between items-center mt-4 text-sm text-gray-600">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => tableData.page > 1 && loadFunc(tableData.page - 1, tableData.limit)}
              ...
              <button
                onClick={() => (tableData.page * tableData.limit < tableData.total) && loadFunc(tableData.page + 1, tableData.limit)}
                className="px-3 py-1 rounded bg-gray-100 hover:bg-gray-200 disabled:opacity-40 transition-colors"
  ...
  return (
    <div className={`min-h-screen watermark-container ${isMobile ? 'p-4' : 'p-8 mx-auto max-w-[1140px]'}`}>
        ...
        <div>
          ...
          {dateRange.start && dateRange.end && (
            <p className="text-sm text-gray-600 mt-2">
              Date Range: {compactRangeLabel(dateRange.start, dateRange.end)}
            </p>
          )}
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 sm:p-6 mb-6">
        ...
        <div className={`flex ${isMobile ? 'flex-col' : 'sm:flex-row sm:items-center'} gap-2 sm:gap-3`}>
          ...
          <button
            onClick={async () => {
              const r = await fetchRange13Months();
              setDateRange({ start: r.data.startDate, end: r.data.endDate });
            }}
